...
import { dbService, UserRole, JobStatus, JobType } from "@/lib/db-service"
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (status === JobStatus.APPROVED) {
      switch (job.type) {
        ...
        case JobType.ANSWER_SUBMISSION:
          if (job.relatedId) {
            ...
            await dbService.updateAnswerStatus(job.relatedId, JobStatus.APPROVED)
          }
          break
      }
      ...
      switch (job.type) {
        case JobType.ANSWER_SUBMISSION:
          if (job.relatedId) {
            // Update answer status
            await dbService.updateAnswerStatus(job.relatedId, JobStatus.REJECTED)
          }
          break
      }
    }
    ...
    return NextResponse.json({ message: "Failed to update job status" }, { status: 500 })
  }
}
