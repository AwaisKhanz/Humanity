...
import { dbService, JobType, JobStatus } from "@/lib/db-service"
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { preNominals, middleInitials, countryOfResidence, bio, links, imageUrl } = await req.json()
    ...
    if (existingProfile) {
      return NextResponse.json({ message: "Author profile already exists" }, { status: 409 })
    }
    ...
    return NextResponse.json({ message: "Failed to create author profile" }, { status: 500 })
  }
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!profile) {
      return NextResponse.json({ message: "Author profile not found" }, { status: 404 })
    }

    return NextResponse.json(profile)
  } catch (error) {
    console.error("Error fetching author profile:", error)
    return NextResponse.json({ message: "Failed to fetch author profile" }, { status: 500 })
  }
}
...
export async function PUT(req: NextRequest) {
  try {
    ...
    // Validate input
    if (!countryOfResidence && !bio && !links && !imageUrl && !preNominals && !middleInitials) {
    ...
    if (!success) {
      return NextResponse.json({ message: "Failed to update author profile" }, { status: 500 })
    }
